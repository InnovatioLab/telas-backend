...
public enum NotificationReference {
    FIRST_SUBSCRIPTION {
        ...
        public EmailDataDto getEmailData(Map<String, String> params) {
            return createEmailDataFirstSub(
    ...
    private static EmailDataDto createEmailDataFirstSub(String subject, String template, Map<String, String> params, String startDate, String endDate) {
        EmailDataDto emailData = new EmailDataDto();
        emailData.setSubject(subject);
        emailData.setTemplate(template);
        emailData.getParams().put("name", params.get("name"));
        ...
        emailData.getParams().put("startDate", startDate);
        emailData.getParams().put("endDate", ObjectUtils.isEmpty(endDate) ? "" : endDate);
        emailData.getParams().put("duration", getDuration(startDate, endDate));
        return emailData;
    }

    private static String getDuration(String startDate, String endDate) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MM/dd/yyyy");
        LocalDate start = LocalDate.parse(startDate, formatter);
        Locale locale = new Locale("en", "US");

        if (ObjectUtils.isEmpty(endDate)) {
            return start.getMonth().getDisplayName(TextStyle.FULL, locale);
        }
        ...
        for (; !current.isAfter(last); current = current.plusMonths(1)) {
            months.add(current.getMonth().getDisplayName(TextStyle.FULL, locale));
        }
